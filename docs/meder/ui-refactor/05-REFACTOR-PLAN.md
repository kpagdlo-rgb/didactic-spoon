...
4. Hide the two single-option selects: render "Data source" and "Diagnostic runtime" as `Select` **only if** more than one option is enabled; otherwise render a read-only chip with the same visible label text (`<span id="source-mode">…`). **Test impact:** `locator("#source-mode option")` and `locator('option[value="model"]')` exist in the suite → keep an actual `<select id="source-mode">` in the DOM but visually replace it with the chip when there is one enabled option (`className="sr-only"` on the select). Same for runtime: keep `<select>` with `<option value="model" disabled>` in DOM. This preserves the test contract and the a11y semantics without showing dead controls.
5. Step 03 panel: render only when `proposal` exists; when it does not and a result exists, show the one-line "No actionable quantity patch." inside the Diagnosis panel instead. Keep the two buttons in the DOM but inside the Diagnosis panel footer (tests query them by role; they remain disabled until a proposal exists).
