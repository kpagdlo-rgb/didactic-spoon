...
- [x] U06: Visual/UX redesign. Audit of the old UI scored 2.2/5 (fixed 1400 px shell with dead side space, 16 simultaneous reassurance sentences, flat typography, no landing page, no motion). Audit, design tokens, component map (Rare UI / paper-design shaders / Lucide), landing spec, Bun-native phased plan and frozen test contract are in [`ui-refactor/`](ui-refactor/README.md). Implemented across five phases (commits e48bce1 → 9ccd4d3): Bun toolchain with verified Bun 1.3.1 gotchas documented (P0); full-bleed 100dvh shell with fluid clamp() typography (P1); Rare UI FluidOrb + exact-string QuantityCounter fork + Lucide icons, access panel collapsed, copy cleanup — AI kicker removed, "total-spend" subtitle removed, all numerotation removed (P2); landing page at `/` with the tool at `/app` and `/meder` redirecting, one permitted test edit (P3); dark theme toggle, mobile scroll-to-result, 390 px overflow fixes (P4). 87 Node tests, typecheck, production build, 20/20 browser tests green; strict CSP preserved (self-hosted fonts, local WebGL, inline SVG).
