...
The Node 24 application pins Next.js 16.3.4, React 19.2.8, and TypeScript 5.9.3 with a Bun-managed lockfile (`bun.lock`); Bun is the package manager and script runner, Next runs under Node. Closed structural validators, a single BigInt/rational solver, versioned controller-owned fixtures, immutable evidence, bounded read-only tools, session-bound create/poll/cancel routes, sanitized export, and explicit deterministic/model controls are implemented. Provider prose is discarded; explanations and verdicts come from the solver.
