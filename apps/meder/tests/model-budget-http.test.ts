...
test('HTTP admission budget is shared across fresh sessions and consumed by canceled and failed runs', async t => {
  const original = { key: process.env.OPENAI_API_KEY, model: process.env.OPENAI_MODEL, budget: process.env.MEDER_MODEL_RUN_BUDGET };
  process.env.OPENAI_API_KEY = 'test-only-placeholder';
  process.env.OPENAI_MODEL = 'test-only-model';
  process.env.MEDER_MODEL_RUN_BUDGET = '2';
  t.after(() => {
    for (const [key, value] of Object.entries({ OPENAI_API_KEY: original.key, OPENAI_MODEL: original.model, MEDER_MODEL_RUN_BUDGET: original.budget })) {
  ...
  const { createDiagnosis, cancelDiagnosis, getDiagnosis, capabilities } = await import('../src/server/http');
  ...
  const first = await createDiagnosis(request(envelope));
  assert.equal(first.status, 200);
  const firstRun = await first.json();
  const cookie = first.headers.get('set-cookie')!.split(';')[0];
  assert.equal((await createDiagnosis(request(envelope))).status, 429, 'a fresh session cannot bypass concurrency');
  ...
  await new Promise(done => setTimeout(done, 10));
  const second = await createDiagnosis(request(envelope));
  ...
  assert.equal((await createDiagnosis(request(envelope))).status, 429, 'a new session cannot refill spent budget');
